...
for k, table in enumerate(tables):
    ...
    for row in rows:
        ...
        scores = [score.strip(")[a]") for score in scores]
        if scores == [""]:
            data_dict.pop(week)
            break
...
for week_num, couples in data_dict.items():
    for couple, scores in couples.items():
        ...
        data_dict[week_num][couple] = judge_scores.copy()
...
with open('JudgeScores.csv', 'w', newline='') as file:
    ...
    for week, couples in data_dict.items():
        for couple, scores in couples.items():
            writer.writerow([week, couple, scores.get(judge_names[0][0], None), scores.get(judge_names[0][1], "N/A"), scores.get(judge_names[0][2], "N/A"), scores.get("Guest Judge", "N/A")])